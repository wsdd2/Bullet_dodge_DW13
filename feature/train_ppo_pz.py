...
- 训练日志：rolling 胜率 / 平均回报 / 平均局长 / 突然死亡比例 / PPO loss 指标
- checkpoint：定期保存策略快照
- best.pt：按指定指标保存 player_0 的最佳策略
- 固定对手模式：加载外部策略并冻结（只训练 player_0）
...
2) 固定对手，只训练 player_0
  python train_ppo_pz.py --num_players 6 --steps 800000 --device cuda \
    --opponent_run_dir runs_pz/20260105_110822 --opponent_source final

3) 训练结束评测并写逐回合日志（高亮 player_0）
  python train_ppo_pz.py --eval_episodes 5
"""
...
import time
import sys
sys.path.append(r"F:/round_godness/PettingZoo V2/")
import json
import argparse
import glob
...
def find_policy_file(run_dir: str, agent: str, kind: str) -> Optional[str]:
    """
    kind:
      - final:          final_player_i.pt 或 player_i.pt
      - checkpoint_latest: checkpoints/ 下最新 upd_*.pt
      - checkpoint_random: checkpoints/ 下随机 upd_*.pt
    """
    if not run_dir:
        return None
    ...
    if not files:
        return None
    if kind == "checkpoint_latest":
        return files[-1]
    if kind == "checkpoint_random":
...
@torch.no_grad()
def eval_logs_best_vs_old(env, obs_dim: int, act_dim: int, device: torch.device, run_dir: str, episodes: int):
    """
    player_0 使用 best.pt
    其余玩家从 run_dir/checkpoints 中随机抽旧策略（若不存在则使用 final）
    逐回合日志写入 run_dir/eval_logs
    """
    ...
    for a in agents:
        ...
        p = find_policy_file(run_dir, a, "checkpoint_random")
        if p is None:
            p = find_policy_file(run_dir, a, "final")
        if p is None:
            # fallback: random init (still runnable)
            net.eval()
            nets[a] = net
            continue
...
def main():
    ...
    # fixed opponents
    ap.add_argument("--opponent_run_dir", type=str, default="", help="固定对手策略来源目录（runs_pz/...）")
    ap.add_argument("--opponent_source", choices=["final", "checkpoint_latest", "checkpoint_random"], default="final")
    ap.add_argument("--train_all", action="store_true", help="启用全员同时学习（忽略 opponent_run_dir）")
    ...
    # load fixed opponents if enabled
    fixed_opponents = (not args.train_all) and bool(args.opponent_run_dir)
    if fixed_opponents:
        for a in agents:
            if a == "player_0":
                continue
            p = find_policy_file(args.opponent_run_dir, a, args.opponent_source)
            if p is None:
                # keep random init
                continue
            nets[a].load_state_dict(torch.load(p, map_location=device))
            nets[a].eval()  # freeze behavior
    ...
    with open(os.path.join(run_dir, "config.json"), "w", encoding="utf-8") as f:
        json.dump({
            ...
            "opponent_run_dir": args.opponent_run_dir,
    ...
    while steps_done < args.steps:
        ...
        if updates_done % args.save_every_updates == 0:
            # always save player_0; when train_all also save others
        ...
        if updates_done % args.print_every_updates == 0:
            ...
            # loss metrics
            if fixed_opponents:
                m0 = update_metrics.get("player_0", {})
                mean_pi = m0.get("pi_loss", 0.0)
                mean_v = m0.get("v_loss", 0.0)
                mean_ent = m0.get("entropy", 0.0)
                mean_clip = m0.get("clip_frac", 0.0)
            else:
    ...
    if args.eval_episodes > 0:
        eval_logs_best_vs_old(env, obs_dim, act_dim, device, run_dir, args.eval_episodes)
